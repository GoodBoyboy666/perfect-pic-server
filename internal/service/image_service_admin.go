...
func (s *ImageService) AdminListImages(params moduledto.AdminImageListRequest) ([]model.Image, int64, int, int, error) {
...
func (s *ImageService) AdminGetImageByID(id uint) (*model.Image, error) {
	image, err := s.imageStore.FindByID(id)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, commonpkg.NewNotFoundError("图片不存在")
		}
...
func (s *ImageService) AdminGetImagesByIDs(ids []uint) ([]model.Image, error) {
