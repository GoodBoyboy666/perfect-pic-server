...
func TestUpdateUsernameAndGenerateToken(t *testing.T) {
	...
	if err != nil || msg == "" || token != "" {
		t.Fatalf("expected validation msg, got token=%q msg=%q err=%v", token, msg, err)
	}
	...
	if err != nil || msg != "用户名已存在" {
		t.Fatalf("expected conflict msg, got msg=%q err=%v", msg, err)
	}

	token, msg, err = UpdateUsernameAndGenerateToken(u.ID, "alice2", true)
	if err != nil || msg != "" || token == "" {
		t.Fatalf("expected success, got token=%q msg=%q err=%v", token, msg, err)
	}
...
func TestUpdatePasswordByOldPassword(t *testing.T) {
	...
	if err != nil || msg == "" {
		t.Fatalf("expected validation msg, got msg=%q err=%v", msg, err)
	}

	msg, err = UpdatePasswordByOldPassword(u.ID, "wrong", "abc123456")
	if err != nil || msg != "旧密码错误" {
		t.Fatalf("expected old password error, got msg=%q err=%v", msg, err)
	}
...
func TestRequestEmailChange(t *testing.T) {
	...
	// Ensure base_url exists to cover URL normalization.
	...
	if err != nil || msg != "密码错误" {
		t.Fatalf("expected password error, got msg=%q err=%v", msg, err)
	}

	msg, err = RequestEmailChange(u.ID, "abc12345", "a@example.com")
	if err != nil || msg != "新邮箱不能与当前邮箱相同" {
		t.Fatalf("expected same email msg, got msg=%q err=%v", msg, err)
	}
	...
	if err != nil || msg != "该邮箱已被使用" {
		t.Fatalf("expected taken msg, got msg=%q err=%v", msg, err)
	}
	...
	// Avoid goroutine flakiness; give it a moment to schedule (email sending is disabled by default settings anyway).
