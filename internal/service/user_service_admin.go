...
func (s *UserService) AdminListUsers(params moduledto.AdminUserListRequest) ([]model.User, int64, error) {
...
func (s *UserService) AdminGetUserDetail(id uint) (*model.User, error) {
	user, err := s.userStore.FindUnscopedByID(id)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, commonpkg.NewNotFoundError("用户不存在")
		}
		return nil, commonpkg.NewInternalError("获取用户详情失败")
	}
	return user, nil
}

// AdminCreateUser 创建后台普通用户。
func (s *UserService) AdminCreateUser(input moduledto.AdminCreateUserRequest) (*model.User, error) {
...
func (s *UserService) AdminPrepareUserUpdates(userID uint, req moduledto.AdminUserUpdateRequest) (map[string]interface{}, error) {
...
func (s *UserService) AdminApplyUserUpdates(userID uint, updates map[string]interface{}) error {
	if len(updates) == 0 {
		return nil
	}
