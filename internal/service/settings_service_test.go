...
func TestInitializeSettings_RemovesLegacySettings(t *testing.T) {
	...
	if err := gdb.Model(&model.Setting{}).
		Where("key = ?", consts.ConfigSiteName).
		Update("value", "My Custom Site Name").Error; err != nil {
		t.Fatalf("update default setting row failed: %v", err)
	}
