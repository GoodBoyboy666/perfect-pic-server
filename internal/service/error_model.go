...
// AuthErrorCode Backward compatibility for existing auth code and tests.
type AuthErrorCode = ErrorCode

const (
	AuthErrorValidation   AuthErrorCode = ErrorCodeValidation
	AuthErrorUnauthorized AuthErrorCode = ErrorCodeUnauthorized
	AuthErrorForbidden    AuthErrorCode = ErrorCodeForbidden
	AuthErrorConflict     AuthErrorCode = ErrorCodeConflict
	AuthErrorNotFound     AuthErrorCode = ErrorCodeNotFound
	AuthErrorInternal     AuthErrorCode = ErrorCodeInternal
)

type AuthError = ServiceError

func newAuthError(code AuthErrorCode, message string) error {
	return NewServiceError(ErrorCode(code), message)
}
