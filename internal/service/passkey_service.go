...
func (s *Service) BeginPasskeyRegistration(userID uint) (string, *protocol.CredentialCreation, error) {
	...
	if err != nil {
		return "", nil, platformservice.NewInternalError("创建 Passkey 注册挑战失败")
	}

	// 服务端保存一次性会话，并仅把 session_id 发给前端。
	sessionID, err := storePasskeySession(passkeySessionRegistration, userID, sessionData)
	if err != nil {
		return "", nil, platformservice.NewInternalError("创建 Passkey 注册会话失败")
	}
...
func (s *Service) FinishPasskeyRegistration(userID uint, sessionID string, credentialJSON []byte) error {
	...
	if err != nil {
		return platformservice.NewValidationError("Passkey 注册校验失败，请重试")
	}
	...
	if err != nil || !isPasskeyAlgorithmAllowed(int64(credentialAlgorithm)) {
		return platformservice.NewValidationError("Passkey 签名算法不被允许")
	}

	credentialID := encodePasskeyCredentialID(credential.ID)
	// 先按 credential_id 查重，区分“同账号重复绑定”和“被其他账号占用”。
	existing, findErr := s.findPasskeyCredentialByCredentialID(credentialID)
	if findErr == nil {
		if existing.UserID == userID {
			return platformservice.NewConflictError("该 Passkey 已绑定")
		}
		return platformservice.NewConflictError("该 Passkey 已被其他账号绑定")
	}
	...
	if err := s.createPasskeyCredential(&model.PasskeyCredential{
		UserID:       userID,
...
func (s *Service) DeleteUserPasskey(userID uint, passkeyID uint) error {
	...
	if err := s.deletePasskeyCredentialByID(userID, passkeyID); err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return platformservice.NewNotFoundError("Passkey 不存在")
		}
		return platformservice.NewInternalError("删除 Passkey 失败")
	}
	return nil
}
...
func (s *Service) UpdateUserPasskeyName(userID uint, passkeyID uint, name string) error {
	...
	if err := s.updatePasskeyCredentialNameByID(userID, passkeyID, normalizedName); err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return platformservice.NewNotFoundError("Passkey 不存在")
		}
		return platformservice.NewInternalError("更新 Passkey 名称失败")
	}
	return nil
}
...
func (s *Service) BeginPasskeyLogin() (string, *protocol.CredentialAssertion, error) {
	...
	if err != nil {
		return "", nil, platformservice.NewInternalError("创建 Passkey 登录挑战失败")
	}

	// 登录挑战同样只在服务端保存明文 SessionData，前端仅持有 session_id。
	sessionID, err := storePasskeySession(passkeySessionLogin, 0, sessionData)
	if err != nil {
		return "", nil, platformservice.NewInternalError("创建 Passkey 登录会话失败")
	}
...
func (s *Service) FinishPasskeyLogin(sessionID string, credentialJSON []byte) (string, error) {
	...
	if err != nil || !isPasskeyAlgorithmAllowed(int64(credentialAlgorithm)) {
		return "", platformservice.NewUnauthorizedError("Passkey 签名算法不被允许")
	}
	...
	if err := s.updatePasskeyCredentialData(
		passkeyUser.userID,
	...
	user, err := s.findUserByID(passkeyUser.userID)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return "", platformservice.NewUnauthorizedError("Passkey 登录失败")
		}
