...
func (s *Service) GenerateForgetPasswordToken(userID uint) (string, error) {
	...
	if redisClient := platformservice.GetRedisClient(); redisClient != nil {
		ctx, cancel := context.WithTimeout(context.Background(), time.Second)
		defer cancel()

		// 保证一个用户只有一个有效 token
		userKey := platformservice.RedisKey("password_reset", "user", strconv.FormatUint(uint64(userID), 10))
		if oldToken, err := redisClient.Get(ctx, userKey).Result(); err == nil && oldToken != "" {
			oldTokenKey := platformservice.RedisKey("password_reset", "token", oldToken)
			_ = redisClient.Del(ctx, oldTokenKey).Err()
		}

		tokenKey := platformservice.RedisKey("password_reset", "token", token)
	...
	if redisClient := platformservice.GetRedisClient(); redisClient != nil {
		...
		if err == nil {
			...
			if parseErr == nil {
				...
				userKey := platformservice.RedisKey("password_reset", "user", strconv.FormatUint(uid, 10))
...
func (s *Service) GenerateEmailChangeToken(userID uint, oldEmail, newEmail string) (string, error) {
	...
	if redisClient := platformservice.GetRedisClient(); redisClient != nil {
		...
		// 保证一个用户只有一个有效 token
		userKey := platformservice.RedisKey("email_change", "user", strconv.FormatUint(uint64(userID), 10))
		if oldToken, err := redisClient.Get(ctx, userKey).Result(); err == nil && oldToken != "" {
			oldTokenKey := platformservice.RedisKey("email_change", "token", oldToken)
			_ = redisClient.Del(ctx, oldTokenKey).Err()
		}
		...
		tokenKey := platformservice.RedisKey("email_change", "token", token)
...
func (s *Service) VerifyEmailChangeToken(token string) (*moduledto.EmailChangeToken, bool) {
	...
	if redisClient := platformservice.GetRedisClient(); redisClient != nil {
		...
		if err == nil && raw != "" {
			...
			userKey := platformservice.RedisKey("email_change", "user", strconv.FormatUint(uint64(payload.UserID), 10))
